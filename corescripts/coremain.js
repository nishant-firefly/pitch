...
var core =
{
    gotoPage: function ($toPage, isNext, index) {
        var currentPage = $toPage.selector;
        var summaryPage = "#HainTableDataTest1";
        var landingPage = "#MWCD_CStore_BlockBuster_Home1"
        ...
        var go_page = function () {
            core.record_navigation_breadcrumb($toPage.attr('id'), function () {
                ...
                var move = -(index * $toPage.width());

                $toPage.addClass('current');
                var doneComplete = false;
                $('#pageholder').find('div.pagediv:visible').transition({
                    x: move + 'px',
                    duration: 300,
                    complete: function () {
                        if (!doneComplete) {
    ...
    gotoSpecificPage: function ($toPage, Pages, index) { //assume you start on page[0], my function is rewriting everything every time.
        if ($toPage.length == 0) return;
        ...
        var go_page = function () {
            core.record_navigation_breadcrumb($toPage.attr('id'), function () {
                ...
                //  $toPage.css('margin-right', '5px');
                var MoveWidth = 0
                for (var i = 1; i < index + 1; i++) // skip 0, it is already displayed.
                {
                    //alert(Pages[i]);
                    //var $thisPage=document.getElementById('#'+Pages[i]);
                    var $thisPage = ($('#' + Pages[i]));
                    //alert($thisPage);
                    //alert($thisPage.width());
                    MoveWidth = MoveWidth + $thisPage.width();
                    //alert(MoveWidth);
                }
                //alert(MoveWidth);
                //var move = -index * MoveWidth-(index*5);
                var move = -MoveWidth;
                //alert(move);
                //alert(index);
                //alert($toPage.width());
...
function set_specific_indicator(LightThisUp) {
    var $liOld = $('#indicator li.selected');
    var $toSelect;
    //alert(LightThisUp);

    $('#indicator li').each(function (index, li) {
        if (LightThisUp == index) {
            //alert(index);
            //alert(LightThisUp);
            $toSelect = $(this);
        }
     ...
     */
    if ($liOld.length == 0)
        $liOld = $('#indicator li:first');
    //alert($liOld.attr('class'));//removeClass('selected').attr('id')
    //$liOld.toggleClass('selected');		// you don't have to do this...
    //$liOld.removeClass('selected');
...
function check_storage_clean_up(visit_key, presentation_key) {
    ...
    save_storage('previousKey', last_visit_key);
...
function core_setup_presentation() {
    ...
    core.record_breadcrumb('Presentation Started: ' + (current_presentation_id == null ? '' : current_presentation_id), function () {
        $('div#left > div').first().click();
    });
}
...
function populate_ui_table($parent_ui) {
    ...
    //var displayCalc = pitch_data.MWM_DisplayCalc_All1;
...
function populate_ui_data($parent_ui) {
    //console.log("populate_ui_data($parent_ui)");
    var $td;
    ...
    //MCASTRO 20180316
    $parent_ui.find('input[filtertarget]').each(function () {
                ...
                $target
                    .find('tr:not(.header) > td[field="' + strFilterField + '"]').each(function () {
                        var $pNode = $(this).prop('parentNode'),
                            description = $(this).prop('innerHTML');
                        var hideThis = arrString.length != 0 && (!description.toLowerCase().includes(arrString[0].toLowerCase()));//comparing the 
    ...
    $parent_ui.find('select[data]').each(function () {
        ...
        if ($element.attr('data') == "MWCD_PC_SelectedFilter") {
            //console.log("$parent_ui.find('select[data]')", data);
        }
...
function plotGraph($divContainerID) {
    ...
    /*KBUDNIKOV 20151201 Find max point in data to be used in xaxis so that when using
     * sideways bar chart it will make the yaxis stay in place.
     * Usage Example: min: -graphMaxMinPoint, max: graphMaxMinPoint
     * */
    var graphMaxMinPoint = 0;
    for (var i = 0; i < seriesValues[0].length; i++) {
        if (seriesValues[0][i] > graphMaxMinPoint) {
            graphMaxMinPoint = seriesValues[0][i];
        }
    }
    ...
    //Catch if any values are zero and there no positive values.
    //If the number is not above zero user the negative value to build x axes
    if (graphMaxMinPoint == 0 || graphMaxMinPoint == 0.0 || graphMaxMinPoint == 0.00) {
        var tempVar = [];
        for (var i = 0; i < seriesValues[0].length; i++) {
            tempVar.push(seriesValues[0][i]);
        }
        // returns the difference between "a" and "b"
        tempVar.sort(function (a, b) {
            return a - b
        });
        graphMaxMinPoint = tempVar[0].toString();
        graphMaxMinPoint = graphMaxMinPoint.replace("-", "");
        graphMaxMinPoint = parseFloat(graphMaxMinPoint);
    }


    //DEBUG
    // console.log("graphMaxMinPoint : " + graphMaxMinPoint);

    var hideZeros = AllValuesSame(seriesValues);
    ...
    else if ($divContainerID.hasClass('mwm_sideways_currency_bar_line_graph')) {
        var options = {
            grid: {
            ...
            axes: {
                ...
                xaxis: {
                    ...
                    ,
                    min: -graphMaxMinPoint,
                    max: graphMaxMinPoint
                }
            }
        }
        ;
    }
    else if ($divContainerID.hasClass('mwm_sideways_percent_bar_line_graph')) {
    ...
    else if ($divContainerID.hasClass('mars_ver_bar_line_graph')) {
        var options = {
            grid: {
                background: 'transparent',
                drawGridlines: false,
            ...
            seriesColors: ['#B7CC5B', '#FCBA30', '#007DC1'],
            axes: {
                xaxis: {
                    ticks: ticks,
                    tickOptions: {
                ...
                yaxis: {
                    ...
                    /*remove the line on the left side of the chart*/
                    rendererOptions: {
                        drawBaseline: false
                    }
    ...
    /*KBUDNIKOV 12/21/2015 - Patch to color negative values on bar graph.
     * TODO: color the negative based of series array rather then on front end.*/

    /*Get all divs with data*/
    var divs = document.getElementsByClassName('jqplot-series-0');
    if (divs != null) {
        var temp;
        //loops through all the divs and then color it red if
        // it has negative sign
        for (var i = 0; i < divs.length; i++) {

            temp = divs[i].innerHTML;
            var test = temp.match(/-\D?/);
...
//Checks to see if all values in Array are the same.
function AllValuesSame(seriesValues) {

    if (seriesValues[0].length == 4) {
        for (var i = 0; i < seriesValues[0].length - 1; i++) {
            if (seriesValues[0][i] != seriesValues[0][i + 1]) {
                return false;
            }
        }
        return true;
